...
class DBConnector
{
    ...
    public function getDistanceArray($raceIds): array
    {
        $results = [];
        $query = "SELECT race_distance  FROM `tbl_hist_results` WHERE `race_id`='".$raceIds."' GROUP BY race_distance";
        $res = $this->dbConnection->query($query);
        while ($result = $res->fetch_object()) {
            $results[] = $result->race_distance;
        }

...
{
    public function debugLogParse(string $searchString, $debugLog)
    {
     ...
     * @param array $distanceNewRankRawData
     * @return array
     */
    public function generateSteps(
        ...
        array $distanceNewRankRawData
    ): array
    {
        $steps = [];

        // initial step:
        $index = $this->arraySearchPartial($generateHandicapRawData, 'AlgorithmStrategyInterface::generateHandicap()');
        if ($index !== null) {
        ...
        $index = $this->arraySearchPartial($generateRankRawData, 'AlgorithmStrategyInterface::generateRank()');
        if ($index !== null) {
            ...
            $steps['f(`rank`)'] = [
                ...
                'emitter' => UpdateHandicapForRaceTask::class
            ];
        }

        // second step: UpdateHandicapForRaceTask
        $index = $this->arraySearchPartial($generateAVGSectionalRawData, 'AlgorithmStrategyInterface::generateAVGSectional()');
        if ($index !== null) {
        ...
        $index = $this->arraySearchPartial($getH2HPointRawData, 'AlgorithmStrategyInterface::getH2HPoint()');
        if ($index !== null) {
        ...
        // fourth step: UpdateDistanceForRaceTask
        $index = $this->arraySearchPartial($distanceNewRankRawData, 'AlgorithmStrategyInterface::distanceNewRank()');
        if ($index !== null) {
            $stepVariables = $this->getVariablesFromDebugLog($distanceNewRankRawData[$index]);

            $ratingTempLine = sprintf(
                "%s %% MIN(HandicapArray)[ArrayKey] = %s",
                $stepVariables->MIN_HANDICAP,
                $stepVariables->RANK
            );
            $steps['f(`distanceNew`)'] = [
                'result' => $stepVariables->RANK,
                'formula' => '[MIN(handicap) % MIN(HandicapArray))[ArrayKey] + 1 = rank',
                'calculation' => $ratingTempLine,
                'emitter' => UpdateRankForRaceTask::class
            ];
        }
    ...
    private function arraySearchPartial(array $arr, string $keyword)
    {

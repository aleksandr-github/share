...
class UpdateRankForRaceTask extends AbstractMySQLTask implements Task
{
    ...
    public function __construct(AlgorithmStrategyInterface $algorithm, object $race, $raceDistance, ?float $positionPercentage)
    {
        ...
        $this->positionPercentage = $positionPercentage ?? $_ENV['positionPercentage'];
                    ...
                    while ($handicap = $handicapResult->fetch_object()) {
                        if ($horsesCount > 0) {
                            ...
                            if ($per > $this->positionPercentage) {
                                ...
                                /**
                                 * This sets up rank to many hist_results at once
                                 * Is this OK?!
                                 *
                                 * After rank is distributed why we're not running
                                 * calculations again?
                                 */
